...
class LocalGlobalRegistration(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        batch_indices = torch.arange(batch_size).to(get_device())
        ...
        ref_indices = torch.arange(ref_length).to(get_device()).view(1, ref_length, 1).expand(batch_size, -1, self.k)  # (B, N, K)
        ...
        src_indices = torch.arange(src_length).to(get_device()).view(1, 1, src_length).expand(batch_size, self.k, -1)  # (B, K, N)
    ...
    def convert_to_batch(ref_corr_points, src_corr_points, corr_scores, chunks):
        ...
        indices = torch.cat([torch.arange(x, y) for x, y in chunks], dim=0).to(get_device())
        ...
        indices = torch.cat([torch.arange(x, y) for x, y in target_chunks], dim=0).to(get_device())
        indices0 = indices.unsqueeze(1).expand(indices.shape[0], 3)  # (total,) -> (total, 3)
        indices1 = torch.arange(3).unsqueeze(0).expand(indices.shape[0], 3).to(get_device())  # (3,) -> (total, 3)

        batch_ref_corr_points = torch.zeros(batch_size * max_corr, 3).to(get_device())
        ...
        batch_src_corr_points = torch.zeros(batch_size * max_corr, 3).to(get_device())
        ...
        batch_corr_scores = torch.zeros(batch_size * max_corr).to(get_device())
