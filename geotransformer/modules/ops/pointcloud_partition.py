...
def get_point_to_node_indices(points: torch.Tensor, nodes: torch.Tensor, return_counts: bool = False):
    ...
    if return_counts:
        ...
        node_sizes = torch.zeros(nodes.shape[0], dtype=torch.long).to(get_device())
...
def point_to_node_partition(
    ...
    node_masks = torch.zeros(nodes.shape[0], dtype=torch.bool).to(get_device())  # (M,)
    node_masks.index_fill_(0, point_to_node, True)

    matching_masks = torch.zeros_like(sq_dist_mat, dtype=torch.bool)  # (M, N)
    point_indices = torch.arange(points.shape[0]).to(get_device())  # (N,)
    ...
    node_indices = torch.arange(nodes.shape[0]).to(get_device()).unsqueeze(1).expand(-1, point_limit)  # (M, K)
    ...
    if return_count:
        ...
        node_sizes = torch.zeros(nodes.shape[0], dtype=torch.long).to(get_device())  # (M,)
