...
def extract_correspondences_from_scores(
    ...
    ref_indices = torch.arange(ref_length).to(get_device())
    ...
    if mutual or bilateral:
        ...
        src_indices = torch.arange(src_length).to(get_device())
...
def get_node_correspondences(
    ...
    if ref_masks is None:
        ref_masks = torch.ones(size=(ref_nodes.shape[0],), dtype=torch.bool).to(get_device())
    if src_masks is None:
        src_masks = torch.ones(size=(src_nodes.shape[0],), dtype=torch.bool).to(get_device())
    if ref_knn_masks is None:
        ref_knn_masks = torch.ones(size=(ref_knn_points.shape[0], ref_knn_points.shape[1]), dtype=torch.bool).to(get_device())
    if src_knn_masks is None:
        src_knn_masks = torch.ones(size=(src_knn_points.shape[0], src_knn_points.shape[1]), dtype=torch.bool).to(get_device())
...
def get_node_overlap_ratios(
    ...
    ref_overlap_masks = torch.zeros(ref_points.shape[0] + 1).to(get_device())  # pad for following indexing
    src_overlap_masks = torch.zeros(src_points.shape[0] + 1).to(get_device())  # pad for following indexing
