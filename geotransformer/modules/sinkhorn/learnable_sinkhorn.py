...
class LearnableLogOptimalTransport(nn.Module):
    ...
    def forward(self, scores, row_masks=None, col_masks=None):
        ...
        if row_masks is None:
            row_masks = torch.ones(size=(batch_size, num_row), dtype=torch.bool).to(get_device())
        if col_masks is None:
            col_masks = torch.ones(size=(batch_size, num_col), dtype=torch.bool).to(get_device())

        padded_row_masks = torch.zeros(size=(batch_size, num_row + 1), dtype=torch.bool).to(get_device())
        padded_row_masks[:, :num_row] = ~row_masks
        padded_col_masks = torch.zeros(size=(batch_size, num_col + 1), dtype=torch.bool).to(get_device())
        ...
        log_mu = torch.empty(size=(batch_size, num_row + 1)).to(get_device())
        log_mu[:, :num_row] = norm.unsqueeze(1)
        log_mu[:, num_row] = torch.log(num_valid_col) + norm
        log_mu[padded_row_masks] = -self.inf

        log_nu = torch.empty(size=(batch_size, num_col + 1)).to(get_device())
